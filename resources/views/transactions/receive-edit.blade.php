...
@section('title', 'Edit Receive Item')

@push('scripts')
<script>
$(document).ready(function() {
    let detailIndex = {{ count($receive->details) }};
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <h5 class="fw-bold mb-0">Edit: {{ $receive->rth_code }}</h5>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('transactions.receive.update', $receive) }}">
            @csrf @method('PUT')
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    <label for="rth_date" class="form-label">Tanggal <span class="text-danger">*</span></label>
                    <input type="date" class="form-control @error('rth_date') is-invalid @enderror" id="rth_date" name="rth_date" value="{{ old('rth_date', $receive->rth_date?->format('Y-m-d')) }}" required>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('tth_id') is-invalid @enderror" id="tth_id" name="tth_id" required>
                        ...
                        @foreach($transfers as $transfer)
                            <option value="{{ $transfer->tth_id }}" {{ old('tth_id', $receive->tth_id) == $transfer->tth_id ? 'selected' : '' }}>
                                {{ $transfer->tth_code }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('branch_id') is-invalid @enderror" id="branch_id" name="branch_id" required>
                        ...
                        @foreach($branches as $branch)
                            <option value="{{ $branch->branch_id }}" {{ old('branch_id', $receive->branch_id) == $branch->branch_id ? 'selected' : '' }}>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('branch_id_from') is-invalid @enderror" id="branch_id_from" name="branch_id_from" required>
                        <option value="">Pilih Branch</option>
                        @foreach($branches as $branch)
                            <option value="{{ $branch->branch_id }}" {{ old('branch_id_from', $receive->branch_id_from) == $branch->branch_id ? 'selected' : '' }}>
                                {{ $branch->branch_code }} - {{ $branch->branch_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('emp_id_receiver') is-invalid @enderror" id="emp_id_receiver" name="emp_id_receiver" required>
                        ...
                        @foreach($employees as $emp)
                            <option value="{{ $emp->emp_id }}" {{ old('emp_id_receiver', $receive->emp_id_receiver) == $emp->emp_id ? 'selected' : '' }}>
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" class="form-control @error('rth_ba_no') is-invalid @enderror" id="rth_ba_no" name="rth_ba_no" value="{{ old('rth_ba_no', $receive->rth_ba_no) }}">
                    @error('rth_ba_no') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-3">
                    <label for="rth_po_no" class="form-label">Purchase No</label>
                    <input type="text" class="form-control @error('rth_po_no') is-invalid @enderror" id="rth_po_no" name="rth_po_no" value="{{ old('rth_po_no', $receive->rth_po_no) }}">
                    @error('rth_po_no') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-3">
                    <label for="rth_notes" class="form-label">Notes</label>
                    <input type="text" class="form-control @error('rth_notes') is-invalid @enderror" id="rth_notes" name="rth_notes" value="{{ old('rth_notes', $receive->rth_notes) }}">
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody id="detailBody">
                        @foreach($receive->details as $idx => $detail)
                            <tr id="detail-row-{{ $idx }}">
                                ...
                                <td>
                                    <input type="number" class="form-control form-control-sm" name="details[{{ $idx }}][rtd_qty]" min="1" value="{{ $detail->rtd_qty }}" placeholder="Qty">
                                </td>
                                <td>
                                    <select class="form-select form-select-sm" name="details[{{ $idx }}][whsl_id_new]">
                                        <option value="">Pilih Warehouse</option>
                                        @foreach($warehouses as $wh)
                                            <option value="{{ $wh->whsl_id }}" {{ $detail->whsl_id_new == $wh->whsl_id ? 'selected' : '' }}>
                                ...
                                <td>
                                    <input type="text" class="form-control form-control-sm" name="details[{{ $idx }}][rtd_notes]" value="{{ $detail->rtd_notes }}" placeholder="Notes">
                                </td>
                                <td>
                                    <button type="button" class="btn btn-sm btn-danger remove-item"><i class="bi bi-trash"></i></button>
                                </td>
