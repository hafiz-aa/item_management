...
@section('title', 'Edit Change Item Description')

@push('scripts')
<script>
$(document).ready(function() {
    let detailIndex = {{ count($change->details) }};
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <h5 class="fw-bold mb-0">Edit: {{ $change->cidh_code }}</h5>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('transactions.change-description.update', $change) }}">
            @csrf @method('PUT')
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    <label for="cidh_date" class="form-label">Tanggal <span class="text-danger">*</span></label>
                    <input type="date" class="form-control @error('cidh_date') is-invalid @enderror" id="cidh_date" name="cidh_date" value="{{ old('cidh_date', $change->cidh_date?->format('Y-m-d')) }}" required>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('branch_id') is-invalid @enderror" id="branch_id" name="branch_id" required>
                        ...
                        @foreach($branches as $branch)
                            <option value="{{ $branch->branch_id }}" {{ old('branch_id', $change->branch_id) == $branch->branch_id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control @error('cidh_notes') is-invalid @enderror" id="cidh_notes" name="cidh_notes" value="{{ old('cidh_notes', $change->cidh_notes) }}">
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody id="detailBody">
                        @foreach($change->details as $idx => $detail)
                            <tr id="detail-row-{{ $idx }}">
                                <td>
                                    ...
                                    <input type="hidden" name="details[{{ $idx }}][masti_id_old]" class="masti-old-input" value="{{ $detail->masti_id_old }}">
                                ...
                                <td>
                                    <input type="text" class="form-control form-control-sm old-description bg-light" readonly value="{{ $detail->oldMaster ? $detail->oldMaster->masti_code . ' - ' . $detail->oldMaster->masti_name : '' }}">
                                </td>
                                <td>
                                    <select class="form-select form-select-sm" name="details[{{ $idx }}][masti_id_new]">
                                        <option value="">Pilih Description</option>
                                        @foreach($masterItems as $mi)
                                            <option value="{{ $mi->masti_id }}" {{ $detail->masti_id_new == $mi->masti_id ? 'selected' : '' }}>{{ $mi->masti_code }} - {{ $mi->masti_name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <input type="text" class="form-control form-control-sm" name="details[{{ $idx }}][cidd_notes]" value="{{ $detail->cidd_notes }}" placeholder="Notes">
                                </td>
                                <td>
                                    <button type="button" class="btn btn-sm btn-danger remove-item"><i class="bi bi-trash"></i></button>
                                </td>
