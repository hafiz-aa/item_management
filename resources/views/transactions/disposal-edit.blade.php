...
$(document).ready(function() {
    let detailIndex = {{ count($disposal->details) }};
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <h5 class="fw-bold mb-0">Edit: {{ $disposal->disph_code }}</h5>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('transactions.disposal.update', $disposal) }}">
            @csrf @method('PUT')
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    <label for="disph_date" class="form-label">Tanggal <span class="text-danger">*</span></label>
                    <input type="date" class="form-control @error('disph_date') is-invalid @enderror" id="disph_date" name="disph_date" value="{{ old('disph_date', $disposal->disph_date?->format('Y-m-d')) }}" required>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('branch_id') is-invalid @enderror" id="branch_id" name="branch_id" required>
                        ...
                        @foreach($branches as $branch)
                            <option value="{{ $branch->branch_id }}" {{ old('branch_id', $disposal->branch_id) == $branch->branch_id ? 'selected' : '' }}>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('disph_sources') is-invalid @enderror" id="disph_sources" name="disph_sources" required>
                        <option value="0" {{ old('disph_sources', $disposal->disph_sources) === '0' ? 'selected' : '' }}>Broken</option>
                        <option value="1" {{ old('disph_sources', $disposal->disph_sources) === '1' ? 'selected' : '' }}>Change Description</option>
                    </select>
                ...
                <div class="col-md-3" id="brokh_id_wrapper">
                    ...
                    <select class="form-select @error('brokh_id') is-invalid @enderror" id="brokh_id" name="brokh_id">
                        ...
                        @foreach($brokenHeaders as $bh)
                            <option value="{{ $bh->brokh_id }}" {{ old('brokh_id', $disposal->brokh_id) == $bh->brokh_id ? 'selected' : '' }}>
                                {{ $bh->brokh_code }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select @error('cust_id') is-invalid @enderror" id="cust_id" name="cust_id" required>
                        ...
                        @foreach($customers as $customer)
                            <option value="{{ $customer->cust_id }}" {{ old('cust_id', $disposal->cust_id) == $customer->cust_id ? 'selected' : '' }}>
                                {{ $customer->cust_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select @error('emp_id_dispossed_by') is-invalid @enderror" id="emp_id_dispossed_by" name="emp_id_dispossed_by">
                        ...
                        @foreach($employees as $emp)
                            <option value="{{ $emp->emp_id }}" {{ old('emp_id_dispossed_by', $disposal->emp_id_dispossed_by) == $emp->emp_id ? 'selected' : '' }}>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control @error('disph_reason') is-invalid @enderror" id="disph_reason" name="disph_reason" value="{{ old('disph_reason', $disposal->disph_reason) }}">
                    @error('disph_reason') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12">
                    <label for="disph_notes" class="form-label">Notes</label>
                    <input type="text" class="form-control @error('disph_notes') is-invalid @enderror" id="disph_notes" name="disph_notes" value="{{ old('disph_notes', $disposal->disph_notes) }}">
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody id="detailBody">
                        @foreach($disposal->details as $idx => $detail)
                            <tr id="detail-row-{{ $idx }}">
                                ...
                                <td>
                                    <input type="number" class="form-control form-control-sm" name="details[{{ $idx }}][dispd_qty]" min="1" value="{{ $detail->dispd_qty }}" placeholder="Qty">
                                </td>
                                <td>
                                    <input type="text" class="form-control form-control-sm" name="details[{{ $idx }}][dispd_notes]" value="{{ $detail->dispd_notes }}" placeholder="Notes">
                                </td>
                                <td>
                                    <button type="button" class="btn btn-sm btn-danger remove-item"><i class="bi bi-trash"></i></button>
                                </td>
