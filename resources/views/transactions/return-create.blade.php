...
@section('title', 'Tambah Return Item')
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <h5 class="fw-bold mb-0">TAMBAH RETURN BARU</h5>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('transactions.return.store') }}">
            @csrf
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    <label for="reth_date" class="form-label">Tanggal <span class="text-danger">*</span></label>
                    <input type="date" class="form-control @error('reth_date') is-invalid @enderror" id="reth_date" name="reth_date" value="{{ old('reth_date', date('Y-m-d')) }}" required>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('issuingh_id') is-invalid @enderror" id="issuingh_id" name="issuingh_id" required>
                        ...
                        @foreach($issuings as $issuing)
                            <option value="{{ $issuing->issuingh_id }}" {{ old('issuingh_id') == $issuing->issuingh_id ? 'selected' : '' }}>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('whsl_id') is-invalid @enderror" id="whsl_id" name="whsl_id">
                        <option value="">Pilih Warehouse</option>
                        @foreach($warehouses as $wh)
                            <option value="{{ $wh->whsl_id }}" {{ old('whsl_id') == $wh->whsl_id ? 'selected' : '' }}>
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" class="form-control @error('reth_ref_no') is-invalid @enderror" id="reth_ref_no" name="reth_ref_no" value="{{ old('reth_ref_no') }}">
                    @error('reth_ref_no') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-3">
                    <label for="reth_ba_no" class="form-label">BA No</label>
                    <input type="text" class="form-control @error('reth_ba_no') is-invalid @enderror" id="reth_ba_no" name="reth_ba_no" value="{{ old('reth_ba_no') }}">
                    @error('reth_ba_no') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-3">
                    <label for="reth_po_no" class="form-label">Purchase No</label>
                    <input type="text" class="form-control @error('reth_po_no') is-invalid @enderror" id="reth_po_no" name="reth_po_no" value="{{ old('reth_po_no') }}">
                    @error('reth_po_no') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-12">
                    <label for="reth_notes" class="form-label">Notes</label>
                    <input type="text" class="form-control @error('reth_notes') is-invalid @enderror" id="reth_notes" name="reth_notes" value="{{ old('reth_notes') }}">
