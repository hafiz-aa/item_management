...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <h5 class="fw-bold mb-0">TAMBAH BROKEN BARU</h5>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('transactions.broken.store') }}">
            @csrf
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    <label for="brokh_date" class="form-label">Tanggal <span class="text-danger">*</span></label>
                    <input type="date" class="form-control @error('brokh_date') is-invalid @enderror" id="brokh_date" name="brokh_date" value="{{ old('brokh_date', date('Y-m-d')) }}" required>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('brokh_status') is-invalid @enderror" id="brokh_status" name="brokh_status" required>
                        <option value="0" {{ old('brokh_status', '0') === '0' ? 'selected' : '' }}>Belum</option>
                        <option value="1" {{ old('brokh_status') === '1' ? 'selected' : '' }}>Proses</option>
                        <option value="2" {{ old('brokh_status') === '2' ? 'selected' : '' }}>Selesai</option>
                    </select>
                    @error('brokh_status') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-3">
                    <label for="brokh_notes" class="form-label">Notes</label>
                    <input type="text" class="form-control @error('brokh_notes') is-invalid @enderror" id="brokh_notes" name="brokh_notes" value="{{ old('brokh_notes') }}">
