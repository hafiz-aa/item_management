...
@section('title', 'Report Disposal Item')
                    ...
                    <tbody>
                        @forelse($rows as $row)
                            <tr>
                                ...
                                <td class="fw-medium">{{ $row->disph_code }}</td>
                                <td>{{ \Illuminate\Support\Carbon::parse($row->disph_date)->format('d/m/Y') }}</td>
                                <td>{{ $row->cust_name ?? '-' }}</td>
                                ...
                                <td>{{ $row->dispd_qty }}</td>
                                <td>
                                    @if($row->disph_is_canceled === '1')
                                        <span class="badge bg-danger">Ya</span>
                                    @else
                                        <span class="badge bg-success">Tidak</span>
                                    @endif
                                </td>
                                <td>{{ $row->dispd_notes ?? '-' }}</td>
                            </tr>
                        @empty
                            <tr>
