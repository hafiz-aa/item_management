        ...
        $.get('{{ route("items.index") }}', { search: search }, function(data) {
            ...
            rows.each(function() {
                ...
                if (cols.length > 1) {
                    ...
                    html += `<div class="search-item px-2 py-1 cursor-pointer" data-id="${id}" style="cursor:pointer">${code} - ${name}</div>`;
                }
            });
            if (html) {
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <h5 class="fw-bold mb-0">TAMBAH TRANSFER BARU</h5>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('transfers.store') }}">
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    ...
                    <input type="date" class="form-control @error('tth_date') is-invalid @enderror" id="tth_date" name="tth_date" value="{{ old('tth_date', date('Y-m-d')) }}" required>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('branch_id_to') is-invalid @enderror" id="branch_id_to" name="branch_id_to" required>
                        <option value="">Pilih Branch</option>
                        @foreach($branches as $branch)
                            <option value="{{ $branch->branch_id }}" {{ old('branch_id_to') == $branch->branch_id ? 'selected' : '' }}>
                                {{ $branch->branch_code }} - {{ $branch->branch_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select @error('emp_id_sender') is-invalid @enderror" id="emp_id_sender" name="emp_id_sender" required>
                        ...
                        @foreach($employees as $emp)
                            <option value="{{ $emp->emp_id }}" {{ old('emp_id_sender') == $emp->emp_id ? 'selected' : '' }}>
                ...
                <div class="col-md-9">
                    ...
                    <input type="text" class="form-control @error('tth_notes') is-invalid @enderror" id="tth_notes" name="tth_notes" value="{{ old('tth_notes') }}">
